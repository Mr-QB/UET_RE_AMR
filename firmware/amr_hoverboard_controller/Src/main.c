...
/*
 * This is a modified version of the hoverboard-firmware-hack project.
 *
 * Modifications:
 * - Added feedback data to the communication packet.
 * - Updated the packet checksum to accommodate the modified packet structure.
 * - Changed the feedback transmission frequency.
 * - Changed the baudrate UART.
 *
 * Modified by: phucnv1206
 * Modification date: 2026-07-22
 *
 * This modified version is licensed under the GNU General Public License
...
int main(void) {
  ...
  while(1) {
    ...
    // ####### FEEDBACK SERIAL OUT #######
    #if defined(FEEDBACK_SERIAL_USART2) || defined(FEEDBACK_SERIAL_USART3)
      if (main_loop_counter % 1 == 0) {    // Send data periodically every 5 ms
        Feedback.start	        = (uint16_t)SERIAL_START_FRAME;
        Feedback.speedR_meas	  = (int8_t)rtY_Right.n_mot;
        Feedback.speedL_meas	  = (int8_t)rtY_Left.n_mot;
#ifdef ENABLE_ODOMETRY
        Feedback.wheelR_cnt     = (int16_t)odom_r;
        Feedback.wheelL_cnt     = (int16_t)odom_l;
#endif
        Feedback.cur = (int16_t)dc_curr;
        Feedback.batVoltage	    = (uint8_t)((batVoltageCalib - 3000) / 4.7);
        Feedback.boardTemp	    = (uint8_t)(board_temp_deg_c / 10);
...
#endif
                                          ^ Feedback.cur ^ Feedback.batVoltage ^ Feedback.boardTemp);
